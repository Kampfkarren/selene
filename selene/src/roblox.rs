...
impl RobloxGenerator {
    pub fn generate(mut self) -> Result<(Vec<u8>, StandardLibrary), Box<dyn Error>> {
        let api: ApiDump = ureq::get(API_DUMP).call()?.into_json()?;
    ...
    pub fn base_std() -> Result<StandardLibrary, Box<dyn Error>> {
        Ok(serde_yaml::from_str(include_str!("./roblox/base.yml"))?)
    }
