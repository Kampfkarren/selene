...
pub struct IncorrectReactUsageLint;
...
impl Lint for IncorrectReactUsageLint {
    type Config = ();
    ...
    fn pass(&self, ast: &Ast, context: &Context, _: &AstContext) -> Vec<Diagnostic> {
        ...
        let mut visitor = IncorrectReactUsageVisitor {
...
fn is_react_create_element(prefix: &ast::Prefix, suffixes: &[&ast::Suffix]) -> bool {
    if_chain! {
...
#[derive(Debug)]
struct IncorrectReactUsageVisitor<'a> {
...
impl<'a> IncorrectReactUsageVisitor<'a> {
...
impl<'a> Visitor for IncorrectReactUsageVisitor<'a> {
    fn visit_function_call(&mut self, call: &ast::FunctionCall) {
        // Check if caller is React.createElement or a variable defined to it
        ...
        if suffixes.is_empty() {
            ...
            // Check if foo is a variable for React.createElement
            ...
            // Call is foo.bar()
            // Check if foo.bar is React.createElement
            check = is_react_create_element(call.prefix(), &suffixes);
            ...
            match field {
                ...
                ast::Field::ExpressionKey { brackets, key, .. } => {
                    ...
                    if_chain::if_chain! {
                        ...
                        if let ast::Prefix::Name(constant_react_name) = var_expression.prefix();
                        if constant_react_name.token().to_string() == "React";
    ...
    fn visit_local_assignment(&mut self, node: &ast::LocalAssignment) {
        for (name, expr) in node.names().iter().zip(node.expressions().iter()) {
            if_chain! {
                ...
                if is_react_create_element(var_expr.prefix(), &var_expr.suffixes().collect::<Vec<_>>());
                then {
...
mod tests {
    ...
    fn test_old_roblox_std() {
        test_lint(
            IncorrectReactUsageLint::new(()).unwrap(),
            "roblox_incorrect_react_usage",
            "old_roblox_std",
        );
    }
